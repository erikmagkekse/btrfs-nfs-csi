...
// --- statInFlight deduplication tests ---

func TestCheckMountHealth_SkipsInFlightStat(t *testing.T) {
	// The globalmount dir does NOT contain a "data" subdir, so normally
	...
	// Simulate an in-flight stat for the data path.
	dataPath := globalmountDir + "/data"
	ns.statInFlight.Store(dataPath, struct{}{})
	defer ns.statInFlight.Delete(dataPath)

	ns.checkMountHealth(context.Background())

	fm := ns.mounter.(*mount.FakeMounter)
	assert.Empty(t, fm.GetLog(), "no mount actions should occur when stat is already in-flight")
}
...
func TestHealMount_Success(t *testing.T) {
	...
	ns := newTestNodeServer([]mount.MountPoint{mp})
	...
	assert.Equal(t, mountPath, actions[0].Target)
...
func TestHealMount_SetsHealthStateHealed(t *testing.T) {
	...
	// Pre-populate abnormal state to verify it gets cleared by heal
	ns.healthState.Store("sc|vol-123", &volumeHealth{abnormal: true, message: "stale"})
...
func TestHealMount_SkipsWhenMountpointGone(t *testing.T) {
	// Mountpoint not registered in FakeMounter -> IsMountPoint returns false
	// simulating Unstage removing the mount while we waited for the lock.
	...
	fm := ns.mounter.(*mount.FakeMounter)
	assert.Empty(t, fm.GetLog(), "no mount actions when mountpoint is gone")
}
