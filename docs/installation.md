...
**Kubernetes:** >= 1.30, VolumeSnapshot CRDs installed, NFSv4.2 client on all nodes
...
AGENT_TENANTS=default:$(openssl rand -hex 16) # tennantName:token,tennant2:pass
...
```bash
TOKEN=$(grep AGENT_TENANTS /etc/btrfs-nfs-csi/agent.env | cut -d: -f2)
